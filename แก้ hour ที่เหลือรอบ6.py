...
try:
    ...
    # 🔄 วิเคราะห์การเปลี่ยนแปลงของ Remaining Hours
    df_trend["Δ Upper"] = df_trend["Upper Avg Hours"].diff().fillna(0).round(2)
    df_trend["Δ Lower"] = df_trend["Lower Avg Hours"].diff().fillna(0).round(2)

    st.subheader("📌 ตารางการเปลี่ยนแปลง Remaining Hours (Δ)")
    st.dataframe(df_trend, use_container_width=True)

    import plotly.graph_objects as go
    st.subheader("📉 กราฟเปรียบเทียบการเปลี่ยนแปลง Remaining Hours (Δ)")

    fig_delta = go.Figure()
    fig_delta.add_trace(go.Bar(x=df_trend["ช่วงชีต"], y=df_trend["Δ Upper"],
                            name="Δ Upper", marker_color='salmon'))
    fig_delta.add_trace(go.Bar(x=df_trend["ช่วงชีต"], y=df_trend["Δ Lower"],
                            name="Δ Lower", marker_color='steelblue'))
    fig_delta.update_layout(barmode='group',
                            title="การเปลี่ยนแปลง Remaining Hours (Δ)",
                            xaxis_title="ช่วงชีต",
                            yaxis_title="Δ Hours")
